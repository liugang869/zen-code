...
class BTreeIndex {
 public:
  ...
  typedef ::mica::transaction::Context<StaticConfig> Context;
 ...
 private:
  ...
  // 为了崩溃恢复重建数据表
  // btree_impl/non_transactional_operator.h

 public:
  template <typename Func>
  uint64_t non_transactional_lookup (const Key& key, const Func& func);
  template <typename Func>
  uint64_t non_transactional_lookup_ptr (const Key& key, const Func& func);
  
  uint64_t non_transactional_insert(Context *ctx, const Key& key, uint64_t value);
  uint64_t non_transactional_update(const Key& key, uint64_t new_value);
  uint64_t non_transactional_remove(Context *ctx, const Key& key, uint64_t value);
  void     non_transactional_check (void);

 private:
  // non transactional insert
  Node* get_node(uint64_t row_id);
  template <bool RightOpen, bool RightExclusive>
  Node* get_node_with_fixup(uint64_t &rah, uint64_t row_id, const Key& key);

  template <bool RightOpen, bool RightExclusive>
  bool fixup_internal(uint64_t &rah, Node*& node_b, const Key& key);

  template <bool RightOpen, bool RightExclusive>
  bool fixup_leaf(uint64_t &rah, Node*& node_b, const Key& key);

  uint64_t insert_recursive(
      Context *ctx, uint64_t &rah, Node* node_b, const Key& key,
      uint64_t value, Key* up_key, uint64_t* up_row_id);

  bool insert_child(Context *ctx, uint64_t &rah,
                    InternalNode* node, const Key& key,
                    uint64_t child_row_id, Key* up_key, uint64_t* up_row_id);
  uint64_t insert_item(Context *ctx, uint64_t &rah,
                       LeafNode* node, const Key& key, uint64_t value,
                       Key* up_key, uint64_t* up_row_id);
  Node* get_writable_node(uint64_t &rah);
  InternalNode* make_internal_node(Context *ctx, uint64_t &rah);
  LeafNode* make_leaf_node(Context *ctx, uint64_t &rah);

  // non transactional update
  template <BTreeRangeType LeftRangeType, BTreeRangeType RightRangeType,
            bool Reversed, bool GetPtr, typename Func>
  uint64_t lookup(const Key& min_key, const Key& max_key,
                  const Func& func);

  template <BTreeRangeType LeftRangeType, BTreeRangeType RightRangeType,
            bool Reversed, bool GetPtr, typename Func>
  uint64_t lookup_recursive(uint64_t &rah,
                            const Node* node_b, const Key& min_key,
                            const Key& max_key, const Func& func);

  template <BTreeRangeType LeftRangeType, BTreeRangeType RightRangeType,
            bool Reversed, bool GetPtr, typename Func>
  uint64_t return_range(uint64_t &rah, const Node* node_b,
                        const Key& min_key, const Key& max_key,
                        const Func& func);
 
  friend DB<StaticConfig>; 
};
}
}
...
#include "btree_index_impl/non_transactional_operator.h"
