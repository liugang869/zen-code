...
void DB<StaticConfig>::print_stats(double elapsed_time,
  ...
  for (auto i=checkpoint_tbls_.begin(); i!= checkpoint_tbls_.end(); i++) {
    printf ("CheckPoint for Table %p\n", i->first);
    i->second->checkpoint_print_status ();
  }
...
void DB<StaticConfig>::print_pool_status() const {
  printf ("memory row version pool:\n");
  ...
  for (uint16_t cls = 0; cls < SharedRowVersionPool<StaticConfig>::kClassCount;
    ...
    for (uint8_t numa_id = 0; numa_id < num_numa_; numa_id++) {
      uint64_t free_rows = aep_shared_row_version_pools_[numa_id]->free_count(cls);
      total_free_rows += free_rows;
      total_rows += aep_shared_row_version_pools_[numa_id]->total_count(cls);
    }

    if (total_rows == 0) continue;

    for (uint16_t thread_id = 0; thread_id < num_threads_; thread_id++) {
      uint64_t free_rows = aep_row_version_pools_[thread_id]->free_count(cls);
      total_free_rows += free_rows;
    }
