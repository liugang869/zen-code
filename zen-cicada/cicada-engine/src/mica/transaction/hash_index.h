...
class HashIndex {
 public:
  ...
  struct Bucket {
    ...
    static constexpr size_t kBucketSize = 3;  // (24 - 8) / 16
    uint64_t next;
  ...
  void print_hash_status () {
    idx_tbl_->print_table_status ();
  }
 ...
 private:
  ...
  //----------------------------------------------------------------------------
  // 以下为我对 非事务 插入/更新 操作的扩展，为了做崩溃恢复阶段的索引恢复
  // 我对这个数据结构的理解
  /*
    ...
    row-1  key(hash-1) bucket->next = x
    row-2  key(hash-2) bucket
    row-3  key(hash-3) bucket
    ...
    row-n  bucket
    ...
    row-x=next_pow_2(n) key(hash-n+1) bucket->next=x+1
    row-x+1 key(hash-2*n+1)

  */
 public:

  // hash_index_impl/non_transactional_operator.h
  template <typename Func>
  ...
  // cas_status is status in RowHead for victim, default 0, when search, lock the bucket first
  template <typename Func>
  uint64_t non_transactional_lookup_ptr_for_multiple_recovery (const Key& key, const Func& func, volatile uint8_t **cas_status);
...
#include "hash_index_impl/non_transactional_operator.h"
