...
bool HashIndex<StaticConfig, UniqueKey, Key, Hash, KeyEqual>::init(
  ...
  for (uint64_t i = 0; i < bucket_count_; i++) {
    ...
    if (!rah.new_row(idx_tbl_, 0, Transaction::kNewRowID, true, kDataSize)) {
      printf("failed to insert bucket - 1\n");
      return false;
    }
    if (rah.row_id() != i) {
      printf("failed to insert buckets - 2\n");
      return false;
    }
    ...
    if (i % kBatchSize == kBatchSize - 1 || i == bucket_count_ - 1) {
      if (!tx->commit()) {
        printf("failed to insert buckets - 3\n");
        return false;
      }
    }

#if defined (RECOVERY) // for multiple thread recovery
    idx_tbl_->head (0, i)->status = 0;
#endif
...
/*
  auto ctx = tx->context();
  for (uint64_t i = 0; i < bucket_count_; i++) {
    uint64_t rowid = ctx->allocate_row (idx_tbl_);
    auto head = idx_tbl_->head (0, rowid);
    auto rv = ctx->allocate_version_for_new_row(idx_tbl_,0,rowid,head,kDataSize);

    auto new_bkt = reinterpret_cast<Bucket*>(rv->data);
    for (uint64_t j = 0; j < Bucket::kBucketSize; j++)
      new_bkt->values[j] = kNullRowID;
    new_bkt->next = kNullRowID;
