...
Table<StaticConfig>::Table(DB<StaticConfig>* db, uint16_t cf_count,
                           ...
                           bool is_index,
                           bool in_aep,
                           bool as_cache, size_t cache_num)
    : db_(db), cf_count_(cf_count), 
      is_index_(is_index),
      in_aep_(in_aep),
      as_cache_(as_cache), 
      cache_num_(std::max(cache_num, 1024UL /*kMinimumCacheNum*/ )) {

  if (in_aep_ == true) {
    assert (as_cache_ == false);
  }

  if (as_cache_ == true) {
    uint16_t thread_cnt = db_->thread_count();
    cache_num_per_thread_ = cache_num_ / thread_cnt;
    cache_num_threshold_per_thread_ = static_cast<size_t>(cache_num_per_thread_)/
                                      StaticConfig::kCacheReplementThreshold;

    assert (::mica::util::is_power_of_two(cache_num_per_thread_));

    victim_mask_ = cache_num_per_thread_-1;
    for (uint16_t i=0; i< thread_cnt; i++) {
      victim_counter_[i] = 0;
      victim_base_[i] = i*cache_num_per_thread_;
    }
  }
  ...
  if (in_aep_) {
    base_root_ = db_->aep_page_pool_->allocate();
  }
  else {
    base_root_ = db_->page_pool_->allocate();
  }
  ...
  if (as_cache_) {
    ...
    while (tmp.size() < cache_num_) {
      allocate_rows (nullptr, tmp);
    }
    std::sort (tmp.begin(), tmp.end());
    printf ("size=%lu cache_num_=%lu per_thread_=%lu front=%lu back=%lu victim_mask=%lu\n",
             tmp.size(), cache_num_, cache_num_per_thread_,
             tmp.front(), tmp.back(), victim_mask_);
    for (uint16_t thread_id=0; thread_id< db_->thread_count(); thread_id++) {
      Context<StaticConfig> *ctx = db_->context (thread_id);
      std::vector<uint64_t> &free_row = ctx->get_free_rows_for_tbl (this);

#if defined (CACHE_QUEUE)
      std::queue<uint64_t> &free_row_queue = ctx->get_free_rows_queue_for_tbl (this);
#endif 

      uint64_t rs = thread_id*cache_num_per_thread_;
      uint64_t re = rs + cache_num_per_thread_-1;
      for (uint64_t r = rs; r<=re; r++) {
        free_row.push_back(r);

#if defined (CACHE_QUEUE)
        free_row_queue.push (r);
#endif

      }
      std::reverse (free_row.begin(), free_row.end());

#if defined (CACHE_QUEUE)
      printf ("thread_id=%u rs=%lu re=%lu free_row.size()=%lu front=%lu back=%lu free_row_queue.size()=%lu front=%lu back=%lu\n", 
               thread_id, rs, re, free_row.size(), free_row.front(), free_row.back(),
               free_row_queue.size(), free_row_queue.front(), free_row_queue.back());
#else
      printf ("thread_id=%u rs=%lu re=%lu free_row.size()=%lu front=%lu back=%lu\n", 
               thread_id, rs, re, free_row.size(), free_row.front(), free_row.back());
#endif
    }
    printf ("\n");
...
Table<StaticConfig>::~Table() {
  if (in_aep_) {
    for (uint64_t i = 0; i < kFirstLevelWidth; i++)
      if (root_[i] != nullptr) db_->aep_page_pool_->free(root_[i]);
    db_->aep_page_pool_->free(base_root_);
  }
  else {
    for (uint64_t i = 0; i < kFirstLevelWidth; i++)
      if (root_[i] != nullptr) db_->page_pool_->free(root_[i]);
    db_->page_pool_->free(base_root_);
  }
}
...
bool Table<StaticConfig>::allocate_rows(Context<StaticConfig>* ctx,
  ...
  if (as_cache_ == true) {
    return false;  // disable when running
  }
  ...
  char* p = nullptr;
  if (in_aep_) {
    p = db_->aep_page_pool_->allocate();
  }
  else {
    p = db_->page_pool_->allocate();
  }
  ...
  if ((row_id >> row_id_shift_) == kFirstLevelWidth) {
    ...
    if (in_aep_) {
      db_->aep_page_pool_->free(p);
    }
    else {
      db_->page_pool_->free(p);
    }
    __sync_lock_release(&lock_);
    return false;
  }
