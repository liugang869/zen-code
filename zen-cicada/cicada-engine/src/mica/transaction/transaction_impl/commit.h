...
#if defined (WBL)  // write behind logging
  {
    t.switch_to(&Stats::logging);
    if (StaticConfig::kVerbose) printf("logging: ts=%" PRIu64 "\n", ts_.t2);
    if (!ctx_->db_->logger()->log(this)) {
...
template <class StaticConfig>
bool AepTransaction<StaticConfig>::aep_begin (bool peek_only) {
  return Transaction<StaticConfig>::begin(peek_only);
}

template <class StaticConfig>
bool AepTransaction<StaticConfig>::aep_commit (Result *result) {
...
template <class StaticConfig>
bool AepTransaction<StaticConfig>::aep_abort (bool skip_backoff) {
  // debug
  // printf ("AepTransaction::aep_abort\n");
...
template <class StaticConfig>
bool AepTransaction<StaticConfig>::aep_commit_item (void) {
...
*/
      ...
      Transaction<StaticConfig>::ctx_->context_pmem_flush (item->write_rv->aep_rv, 
                                       sizeof(AepRowVersion<StaticConfig>)+item->write_rv->data_size);
    }
  }
  ...
  */ 
  ...
  if (mark_at_wset == true) {
    ...
    Transaction<StaticConfig>::ctx_->context_pmem_flush (item->write_rv->aep_rv, sizeof(AepRowVersion<StaticConfig>));
  }
...
template <class StaticConfig>
bool AepTransaction<StaticConfig>::aep_abort_item (void) {
  /* rewrite use iset
