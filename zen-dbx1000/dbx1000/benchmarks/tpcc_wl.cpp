...
void tpcc_wl::init_tab_item() {
	for (UInt32 i = 1; i <= g_max_items; i++) {
		...
		//cly delete: row->set_primary_key(i);
		...
		//cly begin: copy the DRAM row above to NVM
        nvm_row * nvm_row_i;
        // init_tab_item must be run on thread 0
        nvm_row_i = t_item->get_nvm_table()->get_nvm_addr(0);
        row->copy_to_nvm(nvm_row_i, t_item->get_schema()->get_tuple_size(), i, 0);
  		index_insert(i_item, i, nvm_row_i, 0);
        //original: index_insert(i_item, i, row, 0);
        //cly end
	}
}
...
void tpcc_wl::init_tab_wh(uint32_t wid) {
	...
	//cly delete: row->set_primary_key(wid);
	...
	// cly begin: copy the dram row to nvm
    nvm_row * nvm_row_w; 
    nvm_row_w = t_warehouse->get_nvm_table()->get_nvm_addr( (wid-1) % g_thread_cnt );
    row->copy_to_nvm(nvm_row_w, t_warehouse->get_schema()->get_tuple_size(), wid, 0);
	index_insert(i_warehouse, wid, nvm_row_w, wh_to_part(wid));
	// cly end
    return;
}
...
void tpcc_wl::init_tab_dist(uint64_t wid) {
	for (uint64_t did = 1; did <= DIST_PER_WARE; did++) {
		...
		// cly delete: row->set_primary_key(did);
		...
		// cly begin
        nvm_row * nvm_row_d;
        nvm_row_d = t_district->get_nvm_table()->get_nvm_addr((wid-1) % g_thread_cnt);
        row->copy_to_nvm(nvm_row_d, t_district->get_schema()->get_tuple_size(), distKey(did, wid), 0);
		//printf("nvm_row_d addr = %p\n", nvm_row_d);
        index_insert(i_district, distKey(did, wid), nvm_row_d, wh_to_part(wid));
        // cly end
	}
}
...
void tpcc_wl::init_tab_stock(uint64_t wid) {
	...
	for (UInt32 sid = 1; sid <= g_max_items; sid++) {
		...
		// cly delete: row->set_primary_key(sid);
        ...
        // cly begin
        nvm_row * nvm_row_s;
        nvm_row_s = t_stock->get_nvm_table()->get_nvm_addr( (wid-1) % g_thread_cnt );
        row->copy_to_nvm(nvm_row_s, t_stock->get_schema()->get_tuple_size(), stockKey(sid, wid), 0);
		index_insert(i_stock, stockKey(sid, wid), nvm_row_s, wh_to_part(wid));
        // cly end
	}
}
...
void tpcc_wl::init_tab_cust(uint64_t did, uint64_t wid) {
	...
	for (UInt32 cid = 1; cid <= g_cust_per_dist; cid++) {
		...
		// cly delete: row->set_primary_key(cid);
		...
		uint64_t key, sec_key;
        // cly begin
        nvm_row * nvm_row_c;
        nvm_row_c = t_customer->get_nvm_table()->get_nvm_addr((wid-1) % g_thread_cnt);
        row->copy_to_nvm(nvm_row_c, t_customer->get_schema()->get_tuple_size(), custKey(cid, did, wid), 0);
        // cly end
		key = custKey(cid, did, wid);
        sec_key = custNPKey(c_last, did, wid);
        // cly modify: change the secondary index's value to the key of primary index
		index_insert(i_customer_last, sec_key, key, wh_to_part(wid));
		//key = custKey(cid, did, wid);
        // cly modify
		index_insert(i_customer_id, key, nvm_row_c, wh_to_part(wid));
	}
}
...
void tpcc_wl::init_tab_hist(uint64_t c_id, uint64_t d_id, uint64_t w_id) {
    ...
    // cly delete: row->set_primary_key(0);
    // table history is never used. 
...
void tpcc_wl::init_tab_order(uint64_t did, uint64_t wid) {
	...
	for (UInt32 oid = 1; oid <= g_cust_per_dist; oid++) {
        ...
        // table order is never used. 
		// cly delete: row->set_primary_key(oid);
		uint64_t o_ol_cnt = 1;
		...
		// cly begin
        nvm_row * nvm_row_or;
        nvm_row_or = t_order->get_nvm_table()->get_nvm_addr((wid-1) % g_thread_cnt);
        row->copy_to_nvm(nvm_row_or, t_order->get_schema()->get_tuple_size(), orderPrimaryKey(wid, did, oid), 0);
		index_insert(i_order, orderPrimaryKey(wid, did, oid), nvm_row_or, wh_to_part(wid));
		// cly end
        // ORDER-LINE	
#if !TPCC_SMALL
		for (uint32_t ol = 1; ol <= o_ol_cnt; ol++) {
		    ...
		    // cly begin
            nvm_row * nvm_row_ol;
            nvm_row_ol = t_orderline->get_nvm_table()->get_nvm_addr((wid-1) % g_thread_cnt);
            row->copy_to_nvm(nvm_row_ol, t_orderline->get_schema()->get_tuple_size(), orderlineKey(wid, did, oid, ol), 0);
		    index_insert(i_orderline, orderlineKey(wid, did, oid, ol), nvm_row_ol, wh_to_part(wid));
		    // cly end
		}
#endif
		// NEW ORDER
