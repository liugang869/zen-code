...
void * ycsb_wl::init_table_slice() {
	...
	for (uint64_t key = slice_size * tid; 
		...
		// cly delete: new_row->set_primary_key(primary_key);
		new_row->set_value(0, &primary_key);
		Catalog * schema = the_table->get_schema();
        ...
        // cly begin
        nvm_row * nvm_row_t;
        nvm_row_t = the_table->get_nvm_table()->get_nvm_addr(tid);
        new_row->copy_to_nvm(nvm_row_t, the_table->get_schema()->get_tuple_size(), key, part_id);
        // cly end
        ...
        m_item->init();
		m_item->type = DT_row;
        // cly modified new_row to nvm_row_t
		m_item->location = nvm_row_t;
		m_item->valid = true;
		uint64_t idx_key = primary_key;
