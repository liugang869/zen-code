...
void Row_hekaton::init(row_t * row, ts_t ts) {
	_his_len = 4;
	...
	_his_oldest = 0;
    // cly begin
    _write_history[_his_latest].begin = ts;
    _write_history[_his_oldest].begin = ts;
    // cly end
...
// cly begin
void Row_hekaton::free_me() {
    for (uint32_t i = 1; i < _his_len; i++){
        if(_write_history[i].row){
...
Row_hekaton::post_process(txn_man * txn, ts_t commit_ts, RC rc)
{
    ...
    //printf("begin, txnid, commit_ts: %lu, %lu, %lu\n", entry->begin, txn->get_txn_id(), commit_ts);
    //cly temporally delete: assert(entry->begin == txn->get_txn_id());
