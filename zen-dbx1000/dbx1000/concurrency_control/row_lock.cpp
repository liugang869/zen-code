...
// cly begin
void Row_lock::free_me() {
    // free all the entries in the list
    while(waiters_head){
        LockEntry * victim_len = waiters_head;
        LIST_REMOVE_HT(waiters_head, waiters_head, waiters_tail);
        return_entry(victim_len);
    }    
    // free the stack
    LockEntry * victim_sen = NULL;
    STACK_POP(owners, victim_sen);
    while(victim_sen){
        return_entry(victim_sen);
        STACK_POP(owners, victim_sen);
    }
