...
void Row_mvcc::init(row_t * row, ts_t ts) {
	_row = row;
    orig_row = row;
	_his_len = 4;
	_req_len = _his_len;
...
// cly begin
void Row_mvcc::free_me(){
    for (uint32_t i = 0; i < _his_len; i++){
        if(_write_history[i].row && _write_history[i].row != orig_row){
            assert(_write_history[i].row);
            _write_history[i].row->free_row();
            _mm_free(_write_history[i].row);
        }
    }
...
RC Row_mvcc::access(txn_man * txn, TsType type, row_t * row) {
	RC rc = RCOK;
	ts_t ts = txn->get_ts();
    //if(type == P_REQ || type == W_REQ)
        //printf("txn_id: %lu, txn_ts: %lu\n", txn->get_txn_id(), txn->get_ts());
...
INC_STATS(txn->get_thd_id(), debug4, t2 - t1);
	...
	if (type == R_REQ) {
		...
		if (ts < _latest_wts || ts < _max_served_rts || (_exists_prewrite && _prewrite_ts > ts))
			...
			txn->cur_row = res_row;
		    //printf("P: txn_id = %lu, prewrite_id = %u\n", txn->get_txn_id(), _prewrite_his_id);
        /*for (uint32_t i = 0; i < _his_len; i++) {
		    printf("in P_REQ: i valid reserved row: %u, %d, %d, %p\n", 
                i, _write_history[i].valid, _write_history[i].reserved, _write_history[i].row);
        }*/         
		}
	} else if (type == W_REQ) {
		rc = RCOK;
		...
		//printf("W: txn_id = %lu, prewrite_id = %u\n", txn->get_txn_id(), _prewrite_his_id);
        //printf("row == %p, pre_row == %p\n", row, _write_history[_prewrite_his_id].row);
...
Row_mvcc::reserveRow(ts_t ts, txn_man * txn)
{
	...
	if (_num_versions < _his_len) {
		for (uint32_t i = 0; i < _his_len; i++) {
		    //printf("i valid reserved row: %u, %d, %d, %p\n", i, _write_history[i].valid, _write_history[i].reserved, _write_history[i].row);
        	if (!_write_history[i].valid 
        ...
        //printf("_num_versions idx _his_len: %d %d %d\n", _num_versions, idx, _his_len);
		assert(idx < _his_len);
	}
	row_t * row;
	if (idx == _his_len) { 
		if (_his_len >= g_thread_cnt) {
			...
			// printf("enter recycle the oldest version!\n");
