...
public:
    ...
    // cly begin
    void remove_items(idx_key_t key);
    // cly end
...
public:
    ...
    RC          index_replace(idx_key_t key, itemid_t * item, int part_id=-1);
    RC          index_get_latch(idx_key_t key, int part_id=-1);
    RC          index_release_latch(idx_key_t key, int part_id=-1);
    RC          index_replace_without_getting_latch(idx_key_t key, itemid_t *item, int part_id=-1);
    // cly end
private:
