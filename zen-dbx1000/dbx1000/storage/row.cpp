...
void row_t::init_manager(row_t * row, ts_t ts) {
...
#if CC_ALG == MVCC || CC_ALG == HEKATON
    manager->init(this, ts);
#elif CC_ALG != HSTORE
	manager->init(this);
#endif
}
...
#elif CC_ALG == MVCC || CC_ALG == HEKATON || CC_ALG == TICTOC || CC_ALG == SILO
    _mm_free(manager);
#elif CC_ALG == OCC
    mem_allocator.free(manager, sizeof(Row_occ));
#elif CC_ALG == VLL
    mem_allocator.free(manager, sizeof(Row_vll));
#endif
}
...
//cly begin
void row_t::copy_to_nvm(nvm_row * dest_row, uint64_t data_sz, uint64_t row_id, ts_t ts){
    dest_row->row_id = row_id;
    dest_row->nvm_wts = ts;
    memcpy(dest_row->data, this->data, data_sz);
    //TODO: clflush and sfence
}

void row_t::copy_from_nvm(nvm_row * src_row){
    memcpy(this->data, src_row, this->get_tuple_size());
    this->set_primary_key(src_row->row_id);
}
//cly end
