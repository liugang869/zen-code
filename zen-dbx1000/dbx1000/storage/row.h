...
class nvm_row{
 ...
 /* public:
    void set_row_id(uint64_t row_id) {this->row_id = row_id; };
    uint64_t get_row_id() {return row_id; };
    void set_wts(ts_t ts) {this->nvm_wts = ts; };
    ts_t get_wts() {return nvm_wts; };
...
public:
	...
	void init_manager(row_t * row, ts_t ts = 0);
    // cly add
    void free_manager();
    ...
    // cly add
    void        set_part_id(uint64_t pid) {_part_id = pid; };
    ...
    //cly begin
    void copy_to_nvm(nvm_row * dest_row, uint64_t data_sz, uint64_t row_id, ts_t ts);
    void copy_from_nvm(nvm_row * src_row);
    //cly end
