...
{
public:
	virtual void init(thread_t * h_thd, workload * h_wl, uint64_t thd_id); // cly change part_id to thd_id, might be a typo
	void release();
	thread_t * h_thd;
	workload * h_wl;
	myrand * mrand;
    ...
    // cly add INDEX and key
    row_t * 		get_row(row_t * row, access_t type, INDEX * index, idx_key_t key);
    // cly begin
    row_t *         get_row(nvm_row * nvm_row, access_t type, INDEX * index, idx_key_t key, int part_id);
    void            index_insert(INDEX * index, idx_key_t key, row_t * row, int part_id = -1);
    void            set_row_status(RC rc);
    // cly end
protected:	
