...
// cly begin
void workload::index_insert(INDEX * index, uint64_t key, nvm_row * nvm_row, int64_t part_id) {
	uint64_t pid = part_id;
	if (part_id == -1)
		pid = get_part_id(nvm_row);
	itemid_t * m_item =
		(itemid_t *) mem_allocator.alloc( sizeof(itemid_t), pid );
	m_item->init();
	m_item->type = DT_row;
	m_item->location = nvm_row;
	m_item->valid = true;
    assert( index->index_insert(key, m_item, pid) == RCOK );
}

void workload::index_insert(INDEX * index, uint64_t key, uint64_t pri_key, int64_t part_id) {
	uint64_t pid = part_id;
	//if (part_id == -1)
	//	pid = get_part_id(pri_key);
	itemid_t * m_item =
		(itemid_t *) mem_allocator.alloc( sizeof(itemid_t), pid );
	m_item->init();
	m_item->type = DT_row;
	m_item->location = (void *) pri_key;
	m_item->valid = true;
    assert( index->index_insert(key, m_item, pid) == RCOK );
}

void workload::index_remove(INDEX * index, uint64_t key, row_t * row, int64_t part_id) {
    uint64_t pid = part_id;
    if(part_id == -1)
        pid = get_part_id(row);
    assert( index->index_remove(key, pid) == RCOK );
}
// cly end
