...
protected:
    ...
    // cly begin
    void index_insert(INDEX * index, uint64_t key, nvm_row * nvm_row, int64_t part_id = -1);
    void index_insert(INDEX * index, uint64_t key, uint64_t pri_key, int64_t part_id = -1);
    void index_remove(INDEX * index, uint64_t key, row_t * row, int64_t part_id = -1);
    // cly end
};
